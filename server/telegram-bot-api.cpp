...
int main(int argc, char *argv[]) {
  ...
  options.add_option('\0', "enable-file-streaming", "enable the experimental streaming file endpoint",
                     [&] { parameters->file_streaming_enabled_ = true; });
  options.add_checked_option('\0', "file-stream-chunk-size", "streaming file read chunk size in bytes",
                             td::OptionParser::parse_integer(parameters->file_stream_chunk_size_));
  options.add_checked_option('\0', "file-stream-max-connections", "maximum number of active streaming responses",
                             td::OptionParser::parse_integer(parameters->file_stream_max_connections_));
  options.add_option('\0', "file-stream-first-byte-timeout", "streaming first byte timeout in seconds",
                     [&](td::Slice value) { parameters->file_stream_first_byte_timeout_ = td::to_double(value); });
  options.add_option('\0', "file-stream-idle-timeout", "streaming progress timeout in seconds",
                     [&](td::Slice value) { parameters->file_stream_idle_timeout_ = td::to_double(value); });
  ...
  options.add_check([&] {
    ...
    if (parameters->file_stream_first_byte_timeout_ <= 0 || parameters->file_stream_idle_timeout_ <= 0) {
      return td::Status::Error("File stream timeouts must be positive");
    }
  ...
  file_stream_config.max_connections = parameters->file_stream_max_connections_;
  ...
  sched
      ...
      .create_actor_unsafe<HttpServer>(
          ...
          [client_manager, shared_data, file_stream_config] {
            return td::ActorOwn<td::HttpInboundConnection::Callback>(
                td::create_actor<HttpConnection>("HttpConnection", client_manager, shared_data, file_stream_config));
          })
      .release();
