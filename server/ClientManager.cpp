...
void ClientManager::send_file_stream(td::ActorId<FileStreamConnection> stream, td::int64 stream_id, td::string token,
  ...
  if (token.empty() || token[0] == '0' || token.size() > 80u || token.find('/') != td::string::npos ||
    ...
    return fail(401, "Invalid token specified");
  }
  auto colon_pos = token.find(':');
  auto r_user_id = td::to_integer_safe<td::int64>(td::Slice(token).substr(0, colon_pos));
  if (r_user_id.is_error() || r_user_id.ok() <= 0 || r_user_id.ok() >= (static_cast<td::int64>(1) << 54)) {
    return fail(401, "Invalid token specified");
  }
  if (!token_range_(r_user_id.ok())) {
    return fail(421, "Unallowed token specified");
  }
  if (file_id.empty() || file_id.size() > 4096u) {
    return fail(400, "Invalid file_id specified");
  }
  ...
  if (id_it == token_to_id_.end()) {
    ...
    if (!ip_address.empty()) {
      ...
      auto res = flood_controls_.emplace(ip_address, td::FloodControlFast());
      auto &flood_control = res.first->second;
      if (res.second) {
        flood_control.add_limit(60, 20);
        flood_control.add_limit(60 * 60, 600);
      }
      auto now = td::Time::now();
      auto wakeup_at = flood_control.get_wakeup_at();
      if (wakeup_at > now) {
        LOG(INFO) << "Reject file stream Client creation from rate-limited IP " << ip_address;
        return fail(429, "Too many authentication attempts");
      }
      flood_control.add_event(now);
    }
    if (is_global_flood_control_enabled_) {
      auto now = td::Time::now();
      if (global_flood_control_.get_wakeup_at() > now) {
        return fail(429, "Too many authentication attempts");
      }
      global_flood_control_.add_event(now);
    }
    auto tqueue_id = get_tqueue_id(r_user_id.ok(), is_test_dc);
    auto id = clients_.create(ClientInfo{BotStatActor(stat_.actor_id(&stat_)), token_key, tqueue_id,
                                         td::ActorOwn<Client>()});
    auto *client_info = clients_.get(id);
    client_info->client_ = td::create_actor<Client>(PSLICE() << "Client/" << token_key, actor_shared(this, id), token,
                                                    is_test_dc, tqueue_id, parameters_,
                                                    client_info->stat_.actor_id(&client_info->stat_));
    std::tie(id_it, std::ignore) = token_to_id_.emplace(token_key, id);
