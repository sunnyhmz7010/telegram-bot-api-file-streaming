...
void HttpConnectionBase::tear_down() {
  fulfill_write_promises(Status::Error("HTTP connection was closed before queued data was flushed"));
...
void HttpConnectionBase::write_next_with_promise(BufferSlice buffer, Promise<Unit> promise) {
  write_next_noflush(std::move(buffer));
  write_promises_.push_back({queued_offset_, std::move(promise)});
  loop();
}

void HttpConnectionBase::fulfill_write_promises(Status error) {
  while (!write_promises_.empty() && (error.is_error() || write_promises_.front().offset <= written_offset_)) {
    auto write_promise = std::move(write_promises_.front());
    write_promises_.pop_front();
    if (error.is_error()) {
      write_promise.promise.set_error(error.clone());
    } else {
      write_promise.promise.set_value(Unit());
    }
  }
}
...
void HttpConnectionBase::write_error(Status error) {
  ...
  fulfill_write_promises(error.clone());
  state_ = State::Close;
  loop();
}
...
void HttpConnectionBase::loop() {
  ...
  if (can_write_local(fd_)) {
    ...
    if (r.is_error()) {
      ...
      fulfill_write_promises(Status::Error(r.error().public_message()));
      on_error(Status::Error(r.error().public_message()));
    } else {
      written_offset_ += r.ok();
      fulfill_write_promises();
    }
